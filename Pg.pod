...
DBI 特有のフラグ以外を、全て有効にします。
言い換えると、DBD::Pg 特有のフラグ (以下に示している 'pg' で始まるもの全て) を有効にします。
...
(訳注: parse plan:解析計画？ cache ? caches ?)
...
属性が真の値とともに渡された場合も実際の C<PREPARE> 文がすぐに発行されます。
逆に、(こういうことをしたくなるケースは非常にまれだと思いますが)L</pg_prepare_now>
属性に偽の値をセットして渡すと、ただちに C<PREPARE> を発行B<させない>ようになります。
...
ステートメントハンドルはダミーの SQL 文を与えることで作成できますが、あなたが準備したいSQL文と
...
B<pg_type>: 追加情報のデータ型、例えば "character varying(20)"
...
このメソッドはデータベースハンドルに対しても、ステートメントハンドルに対しても呼ぶことができます。
...
クエリが完了するのを単に待つ場合には、C<pg_ready()> を繰り返し呼ぶことはせず、単に
...
Perl の配列は添え字が 0 から始まりますが、Postgresでは 1 から始まることに注意が必要です。
いずれにせよ、配列の最初の要素は、配列の最初の要素として扱われます。
