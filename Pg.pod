...
PostgreSQL は blob をテーブルとは別に、いわゆる object-id を使って扱っていますが、これに対して
Oracle では blob はテーブルと関連しており、全てのテーブルはたった一つの blob
しか持てないという制限があります。これが C<blob_read> メソッドが DATABASE パッケージではなく
...
返されるカラムそれぞれに関する整数値が配列のリファレンスで返されます。
この値は、カラムに NULL が許されているかどうかを示し、0 の場合 NOT NULL、1 の場合 NULL が許されている、2 は不明を示します。
C<execute()> を呼ぶ前には、undef が返されます。
