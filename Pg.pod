...
=for memo XXX いきなりエラーじゃないのか？この最後の2行。プレースホルダ '?' があるのに、
対応する値を execute に渡していない。
...
=for memo XXX S8006 って、08006 の間違いか？
...
=for html See the <a href="http://search.cpan.org/~timb/DBI/DBI.pm#TRACING">DBI
section on TRACING</a> for more information.<br />

=end original

=for text より詳しい情報は、DBI の TRACING のセクションにあります。

=for html より詳しい情報は、<a href="http://search.cpan.org/~timb/DBI/DBI.pm#TRACING">DBI
の TRACING のセクション</a> にあります。<br />
...
ラージオブジェクト関数の古い使い方は deprecated です: C<< $dbh->func(..., 'pg_lo_); >>
...
既存のラージオブジェクトをオープンし、引き続く C<pg_lo_*>
...
このモードで開いたオブジェクトを読み取ると、C<pg_lo_open>
を実行した時に有効だったトランザクションスナップショットの時点の
ラージオブジェクトの内容を読むことになります。

=for memo XXX 原文の C<lo_write> は明らかに C<pg_lo_open> の間違い。
...
=end original
  ...
  $lobjId = $dbh->pg_lo_import_with_oid($filename, $OID);
...
pg_lo_import と同様ですが、指定された OID を object-id として使うように試みます。
指定された数値が 0 の場合は、pg_lo_import と同じ振る舞いに fall back します。
...
=for memo XXX tread carefully は treat carefully か？
...
(訳注: 接続がまだ有効かどうかを調べるには L</ping> メソッドが使えます。)
...
=for memo XXX
ここから prepare の説明が始まるが、このメソッドが何を返すかという記述が無いっぽい。
まあ、説明は要らないのかもしれない。でも、prepare が失敗したら undef になるのか、
失敗したというステートを持ったステートメントハンドルが返されるのか、等々。
...
以下には、より詳細な説明と、いくつかの省略可能なフラグについての説明があります。
...
=for memo XXX 全てのパラメータが指定されない場合ってあるっけ？
...
=for memo XXX parse plan:解析計画？ 実行計画とした
...
シーケンス名(より好ましい)か、テーブル名を(省略可能なスキーマ名とともに)与えることができます。
テーブル名のほうが与えられた場合には DBD::Pg はシーケンス自体を探すことを試みます。
...
PID、そしてオプションの通知パラメータです。このメソッドは、
...
=for memo XXX
原文の event name は「条件通知名」、payload string は「通知パラメータ」とした。
...
=for memo XXX
C<pg_onerow> 属性に "0" を設定しても、"1" を設定している時と同様にカンマ区切りになる模様。
...
=for memo XXX なぜこの例だけ、$sql じゃなくて $statement ?
...
=for memo XXX 原文の Please not は Please note の typo
...
L</pg_lo_creat> と L</pg_lo_open> でモードとして使われる定数。
...
データ型は "sticky" です。あるプレースホルダにデータ型をいったん設定すると、
明示的に他の型を設定しない限り、そのプレースホルダの型として記憶されます。
...
例:
  ...
  ## この2つの引数は、プレースホルダに "varchar" として結び付けられます
  $sth->execute(123, "Merk");

  ## 1つ目のプレースホルダの型を integer 型にリセットします
  $sth->bind_param(1, undef, SQL_INTEGER);

  ## 実行時にパラメータを与えると、上記で与えた "undef" は使われなくなる
  $sth->execute(123, "Merk");

  ## 最初のプレースホルダに、値と型をセットする
  $sth->bind_param(1, 234, { pg_type => PG_TIMESTAMP });

  ## 2つ目のプレースホルダに、値と型をセットする。
  ## 3つ目の引数を書いていないので、デフォルトの "varchar" が使われる
  $sth->bind_param('$2', "Zool");

  ## 上で間違ったデータ型をセットしたのに気がついたので、変更する:
  $sth->bind_param('$1', 234, { pg_type => SQL_INTEGER });

  ## 値もまた間違っていたので、こちらも変更する。
  ## データタイプは sticky なので、変更する必要は無い
  $sth->bind_param(1, 567);

  ## ここで execute すると、567 (integer) と "Zool" (varchar) の値で実行される
  $sth->execute();
...
この機能は、実験的サポートとして提供されます。bind_param_inout
への最初の引数は、プレースホルダの番号でなければなりません。2つ目の引数は、
スカラ変数へのリファレンスでなければなりません。3つ目には、単に 0 を指定してください。
これが実際にやっていることは、戻ってきたカラムの値を、カラムが現れる順に、
変数に代入することだという点に注意してください。例:
...
この例の最後の fetch のあとでは、$foo は新しい値 "223" を持つことになります。
この値を渡す方法はかなりのクセ者で、execute に渡すどんな値でも trump することに注意してください。
これは、値の結びつけができる限り遅く、execute() の段階で行われるので、
bind を実行してからクエリを execute するまでの間に、値を変更することを許します。
ですから、上の例の execute は、これと同じです:

=for memo XXX trump any values.
...
プレースホルダに、配列の値を結び付けます。それぞれの値は L</execute_array>
メソッドの呼び出しで順に使われます。
...
以前に準備しておいたプリペアド文を実行します。常に影響のあった行数を返す
C<UPDATE> 文, C<DELETE> 文, C<INSERT> 文に加え、C<SELECT ... INTO table>
文にも C<execute> メソッドを使用することができます。
...
"prepare/bind/execute" (準備/値の結びつけ/実行) のプロセスは、PostgreSQL 7.4
以降のサーバで大きく変更されました。より多くの情報がありますので、
C<prepare()> や C<bind_param()> のエントリもご覧ください。
...
"undef" 値を結びつけることは、データベースに対して NULL を設定することと同義です。
$DBDPG_DEFAULT の値を結びつけることは、リテラルの文字列 'DEFAULT'
をバックエンドに対して送ることと同義です。プリペアド文で DEFAULT
の使用をサポートするまえの PostgreSQL に対してこのオプションを使うと、
サーバサイドのプリペアド文を無効にすることに注意が必要です。
...
DBD::Pg は、配列を渡すこともサポートしています。単純に配列のリファレンスを渡すと、
DBD::Pg はその配列を、バックエンドへの入力として使用できる形式に、平坦化します。
...
もし、Postgres のバージョン 8.2 以降を使用しているのなら、C<UPDATE>, C<DELETE>, C<INSERT>
文の C<RETURNING> 句が返す値を取り出すために fetch メソッドを使うこともできます。例:
...
指定したハッシュのリファレンスか、L</bind_param_array> メソッドで前に与えておいた要素それぞれに
対して、プリペアド文を実行します。詳細は DBI のドキュメントを見てください。
...
L</execute_array> メソッドの内部で使用されるものです。直接使うことはまれでしょう。
詳細は DBI のドキュメントを見てください。
...
ステートメントハンドルから、次の行のデータを取り出します。
カラムの値を持った配列へのリファレンスを返します。
NULL 値のカラムは、配列では undef として返されます。
...
もうこれ以上行が無い時や、エラーの時は、このメソッドは undef を返します。
返された undef がエラーによるものなのかを調べるために、このメソッドの後で C<< $sth->err >>
を調べる(か、L</RaiseError> 属性を使う)べきです。
...
取り出しのたびに同一のリファレンスが返されますので、引き続く取り出しのあとで使うために、
このリファレンスをどこかに保存することはしないでください。
また、配列の要素も行の取り出しのたびに再利用されますので、要素へのリファレンスを取りたい、
という場合には注意してください。L</bind_columns> も見てください。
...
ステートメントハンドルから残りの全ての行を取り出して、それを含んだ配列の配列のリファレンスを返します。
もう行がない場合には、空の配列のリファレンスが返されます。
...
C<$slice> が配列のリファレンスの場合、このメソッドは、
それぞれの行を配列のリファレンスとして取り出すために、内部で L</fetchrow_arrayref>
を使用します。もし C<$slice> が指す配列が空でない場合、
それは個別のカラムを選択するためのスライスとして扱われます。
(指定できる番号はカラムやパラメータの番号が 1 で始まるのとは異なり、 0 から始まります。)
...
引数が無い場合や、$slice が未定義の場合、このメソッドは空の配列のリファレンスを
渡されたときのように振る舞います。
...
もし C<$slice> がハッシュのリファレンスだった場合は、このメソッドは、
それぞれの行をハッシュのリファレンスとして取り出すために、内部で L</fetchrow_hashref>
を使用します。
...
ステートメントハンドルから全ての行を取り出して、それを含んだハッシュのリファレンスを返します。
DBI のドキュメントに完全な記述があります。
...
DBI に対して、このステートメントハンドルの使用を終えたので、この先使わないことを伝えます。
全ての可能な行を読み取っていないが、もう使わないという場合にだけ必要になります。
...
最後のクエリで何行返されたかを返します。他の多くの DBD モジュールとは対照的に、何行返されたか、
という情報は、C<< $sth->execute >> を呼んだ直後に明らかになっています。L</execute>
メソッド自体が行数を返すということを思い出してください。
ですので、このメソッドはめったに必要とはならないでしょう。
...
Perl の変数や属性を、SELECT 文の出力カラムに結び付けます。
カラムは 1 から数えます。データを取り出すためには、カラムに何かを結びつける必要はありません。
...
省略可能なパラメータ C<\%attr> と C<$bind_type> については、DBI のドキュメントをご覧ください。
...
与えられたリストを使って、SELECT 文のカラムそれぞれに対して L</bind_col> を呼びます。
...
ステートメントハンドルから全ての行を取り出して、各行について C<DBI::neat_list>
を呼ぶことで C<$fh> (デフォルトは F<STDOUT>) に書き出します。
行は C<$lsep> (デフォルトは改行) で区切られ、カラムは C<$fsep> (デフォルトはコンマ) で区切られます。
C<$maxlen> はどれだけの横幅で出力できるかを制御し、デフォルトは 35 です。
...
このメソッドはプロトタイピングやテストのための手軽なユーティリティとして設計されています。
人間が読むことを意図し "neat_list" をフォーマット、文字列の編集をするために使っていますので、
データ転送のアプリケーションで使うことは推奨されません。
...
DBD::Pg によってサポートされています。このメソッドは DBD によって実装されていますが
現在ドキュメントされていません。なので、この先変更があるかもしれません。
...
このメソッドの現在の実装は、Oracle の blob に強く影響されているようですが、
私たちは可能な限り互換性を持つようにトライしました。
Oracle では blob はテーブルと関連しており、全てのテーブルはたった一つの blob
しか持てないという制限があります。
これが C<blob_read> メソッドが DATABASE パッケージではなく
STATEMENT パッケージで bless されていることの理由でしょう。
対して、PostgreSQL は blob をテーブルとは別に、いわゆる object-id を使って扱います。
ここでは、フィールドパラメータがこのオブジェクト id を取り扱うために使われています。
blob を一度に取り出す場合、offset と len 引数は 0 にセットします。

=for memo XXX いまいち自信が無い。
...
C<func> インタフェース経由で扱える blob に関する PostgreSQL 特有の関数についてもご覧ください。
...
より詳しい blob に関する情報や例がありますので、PostgreSQL Programmer's Guide
のラージオブジェクトに関する部分をお読みください。
L<http://www.postgresql.org/docs/current/static/largeobjects.html>.

=for memo XXX 日本語版のURLをつける
...
現在の文で返されるカラムの数。この値は SELECT 文と SHOW 文の場合 (SHOW 文のときは常に C<1>)
また、INSERT, UPDATE, DELETE 文に RETURNING 句がある場合にだけ値が返されます。
C<execute()> を呼ぶ前には、undef が返されます。
...
現在の文のカラム名を含む配列のリファレンスを返します。
この値は SELECT 文と SHOW 文の場合、また、
INSERT, UPDATE, DELETE 文に RETURNING 句がある場合にだけ値が返されます。
C<execute()> を呼ぶ前には、undef が返されます。
...
C<NAME> 属性と同様ですが、カラム名は小文字で返されます。
...
C<NAME> 属性と同様ですが、カラム名は大文字で返されます。
...
C<NAME> と同様ですが、配列ではなく、カラム名のハッシュのリファレンスで返されます。
カラム名がハッシュのキーになり、値は 0 から始まるカラムの順番を示す数です。
C<execute()> を呼ぶ前には、undef が返されます。
...
C<NAME_hash> と同様ですが、カラム名は小文字で返されます。
...
C<NAME_hash> と同様ですが、カラム名は大文字で返されます。

=for memo XXX typo? lower case -> upper case
...
文中のそれぞれのカラムのデータ型が配列のリファレンスで返されます。
C<execute()> を呼ぶ前には、undef が返されます。
...
返されるカラムそれぞれに関する整数値が配列のリファレンスで返されます。
それぞれの値は、C<NUMERIC> カラムの場合は正確にその値、C<CHAR> や C<VARCHAR>
の場合は文字数、そのほかのデータ型に対してはバイト数です。
C<execute()> を呼ぶ前には、undef が返されます。
...
返されるカラムそれぞれに関する整数値が配列のリファレンスで返されます。
この値は返されるカラムの scale (位取り、小数点以下の桁数) を示します。
返されるカラムの型が C<NUMERIC> のときにだけ値が返されます。
C<execute()> を呼ぶ前には、undef が返されます。
...
返されるカラムそれぞれに関する整数値が配列のリファレンスで返されます。
この値は、カラムに NULL が許されているかどうかを示し、0 の場合 NOT NULL、1
の場合 NULL が許されている、2 は不明を示します。
C<execute()> を呼ぶ前には、undef が返されます。
...
このステートメントハンドルが作られた元のデータベースハンドルが返されます。
...
現在プレースホルダに結び付けられている値を含んだハッシュを、リファレンスで返します。
"named parameters" タイプのプレースホルダ (":foo" の形式)
を使用している場合、ハッシュのキーはプレースホルダの名前部分が使われます (コロンは除く)。
"dollar sign numbers" タイプのプレースホルダを使用している場合は、ハッシュのキーは番号です。
ドルマークはつきません。"question mark" タイプを使っている場合は、ハッシュのキーは 1
から始まり各プレースホルダ毎に増えていく整数です。
...
もし L</execute> の前にこのメソッドが呼ばれた場合、以前に渡した値がそのまま返されます。
L</execute> の後に呼ばれた場合は、各値のクォートされたものが返されます。
...
現在プレースホルダに結び付けられている型名を含んだハッシュへのリファレンスを返します。
ハッシュのキーは ParamValues 属性と同じです。
ハッシュの値は、キーと値のペアを1つ含んだハッシュへのリファレンスです。
このハッシュのキーは 'TYPE' か 'pg_type' のどちらかです。
もし、カラムの型が一般の SQL の名前を持つ場合には 'TYPE' が使われます。
もし、カラムの型名が Postgres の型名でしか表現できない場合には 'pg_type' が使われます。
ハッシュの値は、もともと渡された型に対応する内部コードです。
(プレースホルダがまだ結び付けられていない場合は値として undef が返されます。)
ParamTypes の出力は L</bind_param> メソッドに渡しなおすことができます。
...
このデータベースハンドルに対していちばん最近行われた "prepare" メソッドの呼び出し
(失敗した "prepare" も含む) で渡された SQL 文を文字列で返します。
"RaiseError" が有効なところで、例外が起こり、例外ハンドラが $@ をチェックしたときに
C<prepare> が失敗したことがわかった場合に特に便利です。

=for memo XXX 日本語になっとらん
...
DBD::Pg 特有の属性です。いちばん最近の fetch でのタプル (行) の番号を返します。
fetch が行われる前や、全ての行を fetch してしまった後では、ゼロが返されます。
...
DBD::Pg 特有の属性です。(bind_param で) 結び付けられているプレースホルダの数を返します。
...
DBD::Pg 特有の属性です。名前つきプレースホルダの情報をハッシュで返します。
キーはプレースホルダ名で、値は 0 か 1 です。プレースホルダに、
(bind_param で) もう値が割り当てられたかどうかを示します。
...
DBD::Pg 特有の属性です。各カラムに対応する整数を含む配列へのリファレンスを返します。
この整数は、カラムのサイズを byte の単位で表します。可変長のカラムは -1 で示されます。
